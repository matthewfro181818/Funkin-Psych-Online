...
function textTyper.update(self, elapsed)
	...
	if (not isAny(utf8.sub(self.text, self.pos, self.pos), ' ', '\n')) then
		if (self.sound) then
			...
			if songName == "aaaaa" and not runHaxeCode('return game.modchartTweens.exists("zeph_talk_up");') and not runHaxeCode('return game.modchartTweens.exists("zeph_talk_down");') then
				cancelTween('mask1')
				cancelTween('mask2')
				doTweenY('zeph_talk_up', 'mask', (getProperty('mask.y') - 20), 0.075, 'sineIn')
			end
	...
	if (self.pos == utf8len) then
		...
		if songName == "aaaaa" then
			cancelTween("zeph_talk_up")
			cancelTween("zeph_talk_down")
			if (getProperty('mask.y') >= 45) then
				doTweenY('mask1', 'mask', 60, 0.5, 'sineInOut')
			elseif (getProperty('mask.y') < 45) then
				doTweenY('mask2', 'mask', 30, 0.5, 'sineInOut')
			end
		end
		self.finished = true;
...
function diag.endDialogue(self, notify)
	...
	if songName == "aaaaa" then
		runTimer('dialogueExit', 0.2);
	elseif songName == "final-destination" then
	...
	if (curr['_type'] == 'dialogue') then
		...
		function refreshPortrait(jump)
			...
			if (self.onDisplay['char'..pos] ~= curr['portrait']) then
				...
				doTweenAlpha('portraitFadeInC_'..pos, tag, 1, 0.3, 'sineOut');
		...
		if (prev['pos'] == curr['pos']) then
			...
			if (self.onDisplay['char'..pos] == curr['portrait']) then
				doTweenAlpha('portrait'..pos..'Up', tag, 1, 0.15, 'sineOut');
			end
			...
			try(function()
				...
				if (self.onDisplay['char'..opos]) then
					...
					if (curr['alone']) then
						...
						doTweenAlpha('portrait'..opos..'Darken', optag, 0, 0.15, 'sineOut')
					end
				end
			end).catch(function(ex)
				derror('Error: ' .. retval);
		...
		if songName == "aaaaa" then
			if curr['dbox'] == "happy" then
				setProperty('mask.angle', 0)
				playAnim('eye', 4)
				playAnim('mouth', 4)
			elseif curr['dbox'] == "scary" then
				setProperty('mask.angle', 0)
				playAnim('eye', 1)
				playAnim('mouth', 1)
			elseif curr['dbox'] == "worried" then
				setProperty('mask.angle', 0)
				playAnim('eye', 2)
				playAnim('mouth', 2)
			elseif curr['dbox'] == "sad" then
				setProperty('mask.angle', 0)
				playAnim('eye', 3)
				playAnim('mouth', 3)
			elseif curr['dbox'] == "flip" then
				setProperty('mask.angle', 40)
				playAnim('eye', 9)
				playAnim('mouth', 9)
			elseif curr['dbox'] == "surprised_flip" then
				setProperty('mask.angle', 40)
				playAnim('eye', 0)
				playAnim('mouth', 8)
			elseif curr['dbox'] == "scream_flip" then
				setProperty('mask.angle', -40)
				playAnim('eye', 5)
				playAnim('mouth', 5)
			elseif curr['dbox'] == "happy_flip" then
				setProperty('mask.angle', -40)
				playAnim('eye', 7)
				playAnim('mouth', 5)
			elseif curr['dbox'] == "sad_flip" then
				setProperty('mask.angle', 40)
				playAnim('eye', 8)
				playAnim('mouth', 8)
			elseif curr['dbox'] == "angry_flip" then
				setProperty('mask.angle', 40)
				playAnim('eye', 10)
				playAnim('mouth', 10)
			elseif curr['dbox'] == "xd_flip" then
				setProperty('mask.angle', -40)
				playAnim('eye', 6)
				playAnim('mouth', 5)
...
function diag.update(self, elapsed, ender)
	...
	if (self.onDisplay.diagName and not self.ctrlLock) then
		...
		if (songName ~= "aaaaa" and keyJustPressed('back')) or (pressed and mouseOver('btnSkip', mx, my)) then
			if luaSoundExists('music') then
				soundFadeOut('music', 1.5, 0)
			end				
			doTweenAlpha('shadows_appear', 'shadows', 0, 0.8, 'linear')
...
function onCreate()
	if songName == "aaaaa" then
		local start_talk = "True"

		--doTweenAlpha('ttt', 'mask', 0.7, 2, 'sineInOut')
...
function onTweenCompleted(tag) --doTweenAlpha('fwallFadeIn_'..nextlayer..'_'..stime, 'fadeWall', 1, stime/2, 'linear');
	if tag == "mask1" then
		doTweenY('mask2', 'mask', 30, 2, 'sineInOut')
	elseif tag == 'mask2' then
		doTweenY('mask1', 'mask', 60, 2, 'sineInOut')
	end
	if tag == 'zeph_talk_up' then
		doTweenY('zeph_talk_down', 'mask', (getProperty('mask.y') + 40), 0.15, 'sineIn')
	elseif tag == "zeph_talk_down" then
		doTweenY('zeph_talk_up', 'mask', (getProperty('mask.y') - 40), 0.15, 'sineIn')
	end
...
function onEvent(name, value1, value2)
	...
	if (name == 'dialogue.setSkin') then
		diag:setSkin(value1);
	elseif (name == 'stopMus') then
		stopSound('good_menu')
	elseif (name == 'loadDialogue' or name == 'dialogue.load') then
		diag:loadDialogue(value1);
	elseif (name == "EndDial") then
		diag:endDialogue(true);
	elseif (name == 'startDialogue' or name == 'dialogue.start') then
		dverb('startDialogue("'..value1..'", "'..value2..'")');
		diag:startDialogue(value1, value2);		
	elseif (name == 'fd_appear') then
		doTweenAlpha('shadows_appear', 'shadows', 1, 0.8, 'linear')
		doTweenX('shadows_move', 'shadows', -3200, 48, 'linear')
	elseif (name == 'fd_byebye') then
		doTweenAlpha('shadows_appear', 'shadows', 0, 0.8, 'linear')
	elseif (name == "rgbEffect") then
		triggerEvent('rgbEffect', '', '')
	elseif (name == 'sound-fade') then
		if value2 == 'fadeIn' then
			playSound(value1, 0, 'music')
			soundFadeIn('music', 1.7, 0, 1)
		elseif value2 == 'fadeOut' then
			soundFadeOut('music', 1.5, 0)
		end
	elseif name == "Music_Zep" then
		if value1 == "pause" then
			if value2 == "0" then
				pauseSound('menu_intro')
			elseif value2 == "1" then
				pauseSound('good_menu')
				pauseSound('scary_menu')
			end

		elseif value1 == "resume" then
			if value2 == "0" then
				resumeSound('menu_intro')
			elseif value2 == "1" then
				resumeSound('good_menu')
				resumeSound('scary_menu')
			end

		elseif value1 == "fadeOut" then
			if value2 == "0" then
				soundFadeOut('menu_intro', 1.5, 0)
			elseif value2 == "1" then
				soundFadeOut('good_menu', 1.5, 0)
				soundFadeOut('scaru_menu', 1.5, 0)
			end

		elseif value1 == "fadeIn" then
			if value2 == "0" then
				soundFadeIn('menu_intro', 1.5, 0, 1)
			elseif value2 == "1" then
				soundFadeIn('good_menu', 1.5, 0, 1)
				soundFadeIn('scaru_menu', 1.5, 0, 1)
			elseif value2 == "2" then
				stopSound('loop1')
			end

		elseif value1 == "swapMusic" then
			if value2 == "0" then
				setSoundVolume('scary_menu', 1)
				setSoundVolume('good_menu', 0)
			elseif value2 == "1" then
				setSoundVolume('good_menu', 1)
				setSoundVolume('scary_menu', 0)
			end
		end
	end
	end).logErrors().doIt();
